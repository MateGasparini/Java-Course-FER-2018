...
public class Lexer {
	...
	/**
	 * True if escaping in quoted strings is allowed, false otherwise.
	 */
	private boolean escapingAllowed;
	
	/**
	 * Constructor specifying the input data.
	 * 
	 * @param text The specified input data.
	 * @param escapingAllowed True if escaping inside quoted strings is allowed.
	 */
	public Lexer(String text, boolean escapingAllowed) {
		data = text.toCharArray();
		this.escapingAllowed = escapingAllowed;
	}
	...
	private String readQuoted() {
		...
		while (true) {
			if (endReached()) {
				...
				currentIndex ++;
				break;
			} else if (escapingAllowed && followsBackslash()) {
				currentIndex ++;
